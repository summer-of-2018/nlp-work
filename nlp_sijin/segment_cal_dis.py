...
def word_embedding(segwithoutlabel_file, log_file, write_file):
    ...
    model = word2vec.Word2Vec(sentences, sg=1, size=300, window=5, min_count=1, negative=3, sample=0.001, hs=1,
                              workers=4)
...
def model_build(data, ohlabels, test_data, test_labels):
    ...
    try:
        ...
        pass
    hist = model.fit(data, ohlabels, validation_data=(test_data, test_labels), epochs=5, batch_size=BATCH_SIZE,
                     shuffle=True, callbacks=[early_stopping, model_checkpoint], verbose=2)
    # model.load_weights(best_model_path,by_name = True, skip_mismatch = True)
    try:
        model.load_weights(best_model_path, by_name=True, skip_mismatch=True)
    except:
...
train_text = seg_sentence('./实验/summary.txt', './实验/train_data_e2.data', './实验/stopwords.txt',
                          './实验/segwithoutlabel_file.txt')
